...
export class DoctorService extends AccountLifecycleService<DoctorDto> {
  ...
  async createDoctor(data: CreateDoctorDto, actor?: Actor, opts?: AccountPasswordOpts) {
    ...
    try {
      created = await this.createDoctorInTenant(data, email, actor, opts);
    } catch (err) {
    ...
    // Deliberately after the transaction commits, not inside it. Bulk
    // creation suppresses these (placeholder identifiers, shared password
  ...
  private async createDoctorInTenant(data: CreateDoctorDto, email: string, actor?: Actor, opts?: AccountPasswordOpts) {
    return this.prisma.$transaction(async (tx) => {
      ...
      // a fixed/predictable string) -- unless bulk creation supplied the
      // admin-chosen initial password instead. Either way it is returned once
      // below so the administrator creating the account can hand it over; it
      // is never stored or logged in plaintext -- only its bcrypt hash is

...
/**
 * Read-only endpoints for the public OPD waiting-area display. Every route is
 * gated on OPDVisit:read -- the single permission the OPDDisplayOperator role
 * holds -- so this account can never call, skip, transfer, cancel, or edit
 * anything: the backend, not just a hidden button, enforces read-only.
 *
 * The display consumes the SAME queue data as the Queue Manager (via
 * OpdDisplayService -> OpdService.getQueue), never a separate queue.
 */
...
export class OpdDisplayController {
  ...
  async departments() {
    return this.displayService.listDepartments();
  }

  /**
   * One-shot authoritative snapshot. Used for the initial paint and,
   * crucially, after a reconnect -- the client always re-syncs to real
   * backend state rather than trusting anything it held while disconnected.
   */
  @Get('queue')
  @RequirePermission('OPDVisit', 'read')
  async queue(@Query('departmentId') departmentId?: string) {
    if (!departmentId) {
      throw new BadRequestException('departmentId query parameter is required');
    }
    return this.displayService.getSnapshot(departmentId);
  }

  /**
   * Real-time stream of authoritative snapshots for one department, over SSE.
   *
   * Update path: a Queue Manager/Doctor mutation -> QueueMutationInterceptor
   * publishes a hospital-scoped "changed" signal -> this stream (filtered to
   * the viewer's own hospital) re-reads the authoritative queue for the
   * chosen department and pushes a fresh, PII-stripped snapshot -> the TV
   * renders it. No polling; the delay is one DB read.
   *
  ...
  @Sse('stream')
  @RequirePermission('OPDVisit', 'read')
  stream(@Query('departmentId') departmentId?: string): Observable<MessageEvent> {
    if (!departmentId) {
      throw new BadRequestException('departmentId query parameter is required');
    }
    ...
    const snapshots$ = merge(of(null), this.queueEvents.streamForHospital(ctx.hospitalId)).pipe(
      switchMap(() =>
        from(runWithTenant(ctx, () => this.displayService.getSnapshot(departmentId!))).pipe(

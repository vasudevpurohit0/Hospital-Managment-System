...
describe('HospitalsService.resetAllHospitalUserPasswords (bulk onboarding-password reset)', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<HospitalsService>(HospitalsService);
    jest.clearAllMocks();
    mockPlatformPrisma.hospital.findUnique.mockResolvedValue({ id: 'h-1', name: 'Hospital A', schemaName: 'hospital_a' });
  });
  ...
  it('never records the new password value in the audit log', async () => {
    mockTenantClient.user.findMany.mockResolvedValue([{ id: 'u-1', identifier: 'nurse@hospital-a.esic.gov.in' }]);
...
describe('HospitalsService.createHospital (auto-created role roster, minus Doctor)', () => {
  ...
  const mockUserProvisioning = { registerSeededIdentifiers: jest.fn().mockResolvedValue(undefined), provisionAdministrator: jest.fn().mockResolvedValue({ id: 'admin-1', identifier: 'admin@h.esic.gov.in' }) };
  const mockStaffService = { createDefaultRoleAccounts: jest.fn().mockResolvedValue({ created: [], skipped: [], failed: [], createdCount: 0, skippedCount: 0, failedCount: 0 }) };
  ...
  beforeEach(async () => {
    ...
    mockPlatformPrisma.hospital.create.mockResolvedValue({ id: 'h-new', slug: 'new-hospital', schemaName: 'hospital_new_hospital' });
    ...
    mockUserProvisioning.provisionAdministrator.mockResolvedValue({ id: 'admin-1', identifier: 'admin@h.esic.gov.in' });
    mockStaffService.createDefaultRoleAccounts.mockResolvedValue({ created: [], skipped: [], failed: [], createdCount: 0, skippedCount: 0, failedCount: 0 });

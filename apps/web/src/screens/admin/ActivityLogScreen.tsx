...
export const ActivityLogScreen: React.FC<ActivityLogScreenProps> = () => {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, q, actorUserId, action, entityType, status, severity, dateFrom, dateTo]);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          {actorUserId && (
            <button
              onClick={() => clearFilterAndViewUser(null)}
                ...
                {group.items.map((entry) => (
                  ...
                  >
                    <div className="flex flex-col items-center pt-1">
                      <span
                        className={`w-2.5 h-2.5 rounded-full ${SEVERITY_DOT[entry.severity]}`}
                      />
                    </div>
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button
                            onClick={() =>
                              entry.actorUserId && clearFilterAndViewUser(entry.actorUserId)
                            }
                      ...
                      <div className="flex items-center gap-3 flex-wrap text-xs text-[var(--color-text-tertiary)]">
                        <span className="flex items-center gap-1">
                          <Monitor className="w-3 h-3" /> {entry.browser ?? 'Unknown'} ·{' '}
                          {entry.os ?? 'Unknown'}
                        </span>
                        {entry.ipAddress && (
                          <span className="flex items-center gap-1">
                        ...
                        {entry.changedFields.length > 0 && (
                          <Badge variant="info">
                            ...
                            {entry.changedFields.length === 1 ? '' : 's'} changed
                          </Badge>
                        )}
                      </div>
      ...
      {selectedEntry && (
        <AuditLogDetailsModal entry={selectedEntry} onClose={() => setSelectedEntry(null)} />
      )}
    </div>
  );
};

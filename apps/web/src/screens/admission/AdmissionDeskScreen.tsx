...
export const AdmissionDeskScreen: React.FC<AdmissionDeskScreenProps> = ({ authToken }) => {
  ...
  const handleOpenAllocation = async (admission: AdmissionRecord) => {
    ...
    setError(null);
    try {
      const [beds, doctorUsers] = await Promise.all([
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-4 sm:p-6 space-y-6">
      {/* Stacks below `sm` so the Refresh control drops under the heading
          rather than overlapping it on a phone. */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
